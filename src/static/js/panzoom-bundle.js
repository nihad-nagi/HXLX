...
class Component {
  ...
  update(patch = {}, { fromProps = false } = {}) {
    ...
    if (
      rendered !== undefined &&
      (!this.shouldRender || this.shouldRender(phase) !== false)
    ) {
  ...
  mount() {
    ...
    if (
      rendered !== undefined &&
      (!this.shouldRender || this.shouldRender("mount") !== false)
    ) {
      this.el.innerHTML = rendered;
    }
    this.bindEvents();
...
class SVGPanZoom extends Component {
  ...
  setupEventListeners() {
    ...
    this._viewport.addEventListener("wheel", this.handleWheel.bind(this), {
      passive: false,
    });
  ...
  handleWheel(e) {
    ...
    const scaleFactor =
      e.deltaY > 0 ? 1 / this.props.scaleFactor : this.props.scaleFactor;
  ...
  fitToView() {
    ...
    const tx =
      (this.state.viewerWidth - bbox.width * scale) / 2 - bbox.x * scale;
    const ty =
      (this.state.viewerHeight - bbox.height * scale) / 2 - bbox.y * scale;
...
class MermaidViewer {
  ...
  setupPanZoom(wrapper, container, svg) {
    ...
    const enterFullscreen = () => {
      ...
      // Hide controls temporarily during transition
      ...
      // Fit to new container size with smooth animation
      setTimeout(() => {
        fitToContainer();
        if (controls) {
          controls.style.opacity = "1";
        }
      }, 10); // Small delay to ensure dimensions are updated
    };
    ...
    const exitFullscreen = () => {
      ...
      // Fit to original container size
      setTimeout(() => {
        fitToContainer();
      }, 10);
    };
    ...
    // Set initial size based on SVG dimensions with margin for controls
    const svgRect = svg.getBBox();
    const margin = 100; // Space for controls
    container.style.width = `${Math.max(300, svgRect.width + margin)}px`;
    container.style.height = `${Math.max(200, svgRect.height + margin)}px`;
