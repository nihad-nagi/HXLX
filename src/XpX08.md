...
```mermaid
flowchart TD
    A[User loads module with params] --> B{Configuration Parser}
    B --> C[Create device instance]
    C --> D[Register with V4L2 subsystem]
    D --> E["/dev/videoX created"]
    F[Application opens /dev/videoX] --> G{V4L2 API Handler}
    G --> H[Process ioctl calls]
    H --> I[Update device state]
    I --> J{Buffer Management}
    J --> K[Allocate/queue buffers]
    K --> L[Generate synthetic frames]
    L --> M[Frame data ready]
    M --> N[Application reads/writes]
    N --> O[Complete V4L2 workflow]
    P[Debug features] --> Q[Log to kernel]
    Q --> R[Export to debugfs]
...
subgraph UserSpace["User Space"]
    AppIn["Video Input<br/>/dev/video0"]
    AppOut["Video Output<br/>/dev/video1"]
end
subgraph KernelSpace["Kernel Space"]
    subgraph V4L2Core["V4L2 Core"]
        VDevIn["video_device<br/>cdev + v4l2_device"]
        VDevOut["video_device<br/>cdev"]
        Sub1["v4l2_subdev"]
        Sub2["v4l2_subdev"]
    end
end
subgraph Hardware["Hardware"]
    subgraph Chip["SoC Modules"]
